...
\ee
where $\probr (\pco, \pnu)$ denotes the prior probability (non-informative priors for the case of this paper).


In particular, we consider the following scenarios:
\begin{itemize}
\item Study the science gain when extending the survey to 10,000 deg$^2$, instead if 2,200 deg$^2$
\item Study the impact of uncertainties in shape measurements and photo-z measurements in combination
\item Study the impact of uncertainties in shape measurements only
\item Study the impact of uncertainties in photo-z estimation only
\item Study the impact of uncertainties in baryonic physics modeling 
\item Study the impact of uncertainties in intrinsic alignment modeling 
\end{itemize}
